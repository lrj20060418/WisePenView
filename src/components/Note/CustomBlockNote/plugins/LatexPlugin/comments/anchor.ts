...
export const MATH_BLOCK_COMMENT_OWNER_ID = 'latex.block.math';
export const INLINE_MATH_COMMENT_OWNER_ID = 'latex.inline.inlineMath';
...
export function getFormulaThreadAnchorsYMap(doc: Doc) {
  return doc.getMap<unknown>(FORMULA_THREAD_ANCHORS_MAP);
}

export function parseFormulaThreadAnchor(value: unknown): FormulaThreadAnchor | null {
  const anchor =
...
export function isSameFormulaThreadAnchor(
  left: FormulaThreadAnchor,
  right: FormulaThreadAnchor
): boolean {
  return (
